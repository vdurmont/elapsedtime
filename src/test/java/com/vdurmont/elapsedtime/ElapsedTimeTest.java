...
public class ElapsedTimeTest {
    ...
    /**
     * Making sure that all locales define all the keys
     */
    @Test public void test_locales() {
        for (ElapsedTime.Locale locale : ElapsedTime.Locale.values()) {
            for (ElapsedTime.StringKey key : ElapsedTime.StringKey.values()) {
                assertNotNull(locale.getString(key));
            }
        }
    }
    ...
    @Test public void get_with_a_different_locale() {
        long duration = 900 * YEAR;
        String result = ElapsedTime.getFromDurationMillis(duration, ElapsedTime.Locale.FRENCH);
        assertEquals("Il y a 900 ans", result);
    }
}
